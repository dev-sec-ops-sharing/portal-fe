...
// Security Vulnerability 1: Hardcoded Stripe Secret API Key (triggers Gitleaks & SonarCloud)
const STRIPE_SECRET_KEY = "sk_test_51NxyzaBc123XYZ456SecretDemoTokenOnlyForScanning";
...
export default function TransactionsPage({ searchParams }: PageProps) {
  ...
  // Log Stripe Key (Bad Practice - prints secrets to execution logs)
  console.log("Initializing Stripe Client with key:", STRIPE_SECRET_KEY);

  // Mock transaction list
  const transactions = [
    { id: "TX_1001", customer: "John Doe", amount: 250000, status: "SUCCESS", date: "2026-07-19" },
    { id: "TX_1002", customer: "Alice Smith", amount: 1200000, status: "PENDING", date: "2026-07-20" },
    { id: "TX_1003", customer: "Bob Johnson", amount: 450000, status: "FAILED", date: "2026-07-20" }
  ];
  ...
  return (
    <main className="flex min-h-screen flex-col bg-slate-50 p-12">
      <div className="max-w-4xl w-full mx-auto space-y-8">
        ...
        {/* Security Vulnerability 2: Reflecting raw search query parameter directly into innerHTML without sanitization (triggers DOM XSS) */}
        {query && (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {transactions
                .filter(t => t.customer.toLowerCase().includes(query.toLowerCase()))
                .map(t => (
